...
class Student:
    def __init__(self, root):
        ...
        delete_btn=Button(btn_frame, text="Delete",command=self.delete_data,font=("times new roman", 12, "bold"), bg="blue", fg="white", width=15)
        ...
        take_photo_btn=Button(btn_frame, text="Take Photo Sample",command=self.generate_dataset,font=("times new roman", 12, "bold"), bg="blue", fg="white", width=15)
        ...
        if self.var_std_id.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = connection.connect(host="localhost",user="root", database="student1", use_pure=True)
                    my_cursor=conn.cursor()
                    sql="delete from StudentDetails where Studentid=%s"
    ...
    def generate_dataset(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get=="":
            ...
            try:
                ...
                self.reset_data()
                conn.close()
                face_classifier=cv2.CascadeClassifier("hass.xml")
                ...
                while True:
                    ...
                    if face_cropped(my_frame) is not None:
                        ...
                        cv2.putText(face,str(img_id),(40,40),cv2.FONT_HERSHEY_COMPLEX,2,(0,255,255),2)
